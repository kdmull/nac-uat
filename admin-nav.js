 ...
 * dependency on supabase-js or shared.js, no global variables (IIFE).
...
(function(){
  ...
  var ADMIN_LINKS = [
    ...
    { href:'dupr-matches.html', label:'DUPR' }
  ];
  ...
  function enforceDuprLink(prof){
    ...
    if(current === 'dupr-connect.html' || current === 'auth.html') return false;
  ...
  function injectLinks(){
    ...
    BASE_LINKS.forEach(function(l){
      ...
      // in the nav (e.g. the brand logo also points to index.html).
  ...
  function injectAccountButtons(){
    ...
    if(current !== 'dashboard.html'){
      ...
      prof.href = 'dashboard.html'; prof.textContent = 'My Profile';
  ...
  function checkPartnerInvites(sess){
    var current = (location.pathname.split('/').pop() || '').toLowerCase();
    if(current === 'dashboard.html') return;   // dashboard has its own banner
